...
def log_actor_material(actor, label: str) -> None:
    ...
    if comp is None:
        print(f"OPERATOR_STAGE3_CAPTURE_MATERIAL label={label} material=none")
        return
    material = comp.get_material(0)
    print(
        "OPERATOR_STAGE3_CAPTURE_MATERIAL "
        f"label={label} material={material.get_name() if material else 'none'}"
    )


def log_stage3_scene_evidence() -> None:
    ...
    for actor in actors:
        try:
            ...
            pass
    stage3_labels = [label for label in labels if "OperatorStage3" in label or "TrafficReadableQueueZone" in label]
    print(f"OPERATOR_STAGE3_CAPTURE_ACTORS total={len(actors)} stage3={len(stage3_labels)}")
    required_labels = [
    ...
    for label in required_labels:
        ...
        if actor is None:
            print(f"OPERATOR_STAGE3_CAPTURE_ACTOR_MISSING label={label}")
            raise SystemExit(f"required Stage 3 actor label missing: {label}")
        try:
            loc = actor.get_actor_location()
            scale = actor.get_actor_scale3d()
            print(
                "OPERATOR_STAGE3_CAPTURE_ACTOR "
                f"label={label} loc={describe_vector(loc)} scale={describe_vector(scale)}"
            )
        except Exception as exc:
            print(f"OPERATOR_STAGE3_CAPTURE_ACTOR label={label} bounds_error={exc}")
        log_actor_material(actor, label)


def main() -> None:
    output = os.environ.get("SMART_INTERSECTION_OPERATOR_STAGE3_PROOF_OUTPUT")
    if not output:
        raise SystemExit("SMART_INTERSECTION_OPERATOR_STAGE3_PROOF_OUTPUT is required")
    ...
    sun.set_actor_label("OperatorStage3_capture_daylight")
    ...
    sky.set_actor_label("OperatorStage3_capture_skylight")
    ...
    pp.set_actor_label("OperatorStage3_capture_postprocess")
    ...
    print(
        "OPERATOR_STAGE3_CAPTURE_CAMERA "
    ...
    capture.set_actor_label("OperatorStage3_scene_capture_operator_proof")
    ...
    print(f"OPERATOR_STAGE3_RENDER_TARGET_EXPORTED map={map_path} output={output_path} ok={ok}")
