...
if (-not $Before) {
  $Before = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage7-before.png'
}
if (-not $After) {
  $After = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage7-production-photoreal-proof.png'
}
if (-not $ContactSheet) {
  $ContactSheet = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage7-before-after-contact-sheet.png'
}
$BeforePath = [System.IO.Path]::GetFullPath($Before)
$AfterPath = [System.IO.Path]::GetFullPath($After)
$ContactSheetPath = [System.IO.Path]::GetFullPath($ContactSheet)

foreach ($path in @($BeforePath, $AfterPath, $ContactSheetPath)) {
...
$PythonScript = Join-Path $RepoRoot 'renderer\unreal\SmartIntersection\Content\Python\capture_operator_map_stage7.py'
...
function Set-Stage7PngOpaque {
  ...
  if (-not ('OperatorStage7PngAlphaFixer' -as [type])) {
...
public static class OperatorStage7PngAlphaFixer
{
...
'@
  ...
  [OperatorStage7PngAlphaFixer]::ForceOpaque($Path)
  Write-Output "OPERATOR_STAGE7_PROOF_OPAQUE output=$Path"
}

function New-Stage7ContactSheet {
  param(
  ...
  Write-Output "OPERATOR_STAGE7_CONTACT_SHEET=$OutputPath"
}

$env:SMART_INTERSECTION_OPERATOR_STAGE7_BEFORE_OUTPUT = $BeforePath
$env:SMART_INTERSECTION_OPERATOR_STAGE7_AFTER_OUTPUT = $AfterPath
$env:SMART_INTERSECTION_OPERATOR_STAGE7_CONTACT_SHEET = $ContactSheetPath
...
foreach ($path in @($BeforePath, $AfterPath)) {
  ...
  Set-Stage7PngOpaque -Path $path
}
New-Stage7ContactSheet -LeftPath $BeforePath -RightPath $AfterPath -OutputPath $ContactSheetPath
Set-Stage7PngOpaque -Path $ContactSheetPath
Write-Output "OPERATOR_STAGE7_BEFORE_OUTPUT=$BeforePath"
Write-Output "OPERATOR_STAGE7_AFTER_OUTPUT=$AfterPath"
