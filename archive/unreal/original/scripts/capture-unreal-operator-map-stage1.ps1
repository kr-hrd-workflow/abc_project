...
if (-not $Output) {
  $Output = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage1-proof.png'
}
...
$PythonScript = Join-Path $RepoRoot 'renderer\unreal\SmartIntersection\Content\Python\capture_operator_map_stage1.py'
...
function Set-OperatorStage1PngOpaque {
  ...
  if (-not ('OperatorStage1PngAlphaFixer' -as [type])) {
...
public static class OperatorStage1PngAlphaFixer
{
...
'@
  ...
  [OperatorStage1PngAlphaFixer]::ForceOpaque($Path)
  Write-Output "OPERATOR_STAGE1_PROOF_OPAQUE output=$Path"
}

$env:SMART_INTERSECTION_OPERATOR_STAGE1_PROOF_OUTPUT = $OutputPath
...
Set-OperatorStage1PngOpaque -Path $OutputPath
Write-Output "OPERATOR_STAGE1_PROOF_OUTPUT=$OutputPath"
