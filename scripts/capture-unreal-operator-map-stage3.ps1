...
if (-not $Output) {
  $Output = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage3-proof.png'
}
...
$PythonScript = Join-Path $RepoRoot 'renderer\unreal\SmartIntersection\Content\Python\capture_operator_map_stage3.py'
...
function Set-OperatorStage3PngOpaque {
  ...
  if (-not ('OperatorStage3PngAlphaFixer' -as [type])) {
...
public static class OperatorStage3PngAlphaFixer
{
...
'@
  ...
  [OperatorStage3PngAlphaFixer]::ForceOpaque($Path)
  Write-Output "OPERATOR_STAGE3_PROOF_OPAQUE output=$Path"
}

$env:SMART_INTERSECTION_OPERATOR_STAGE3_PROOF_OUTPUT = $OutputPath
...
Set-OperatorStage3PngOpaque -Path $OutputPath
Write-Output "OPERATOR_STAGE3_PROOF_OUTPUT=$OutputPath"
