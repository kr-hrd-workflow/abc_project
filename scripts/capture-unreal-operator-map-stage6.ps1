...
if (-not $Before) {
  $Before = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage6-before.png'
}
if (-not $After) {
  $After = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage6-photoreal-proof.png'
}
if (-not $ContactSheet) {
  $ContactSheet = Join-Path $RepoRoot 'artifacts\unreal-operator-map-stage6-before-after-contact-sheet.png'
}
...
$PythonScript = Join-Path $RepoRoot 'renderer\unreal\SmartIntersection\Content\Python\capture_operator_map_stage6.py'
...
function Set-Stage6PngOpaque {
  ...
  if (-not ('OperatorStage6PngAlphaFixer' -as [type])) {
...
public static class OperatorStage6PngAlphaFixer
{
...
'@
  ...
  [OperatorStage6PngAlphaFixer]::ForceOpaque($Path)
  Write-Output "OPERATOR_STAGE6_PROOF_OPAQUE output=$Path"
}

function New-Stage6ContactSheet {
  param(
  ...
  Write-Output "OPERATOR_STAGE6_CONTACT_SHEET=$OutputPath"
}

$env:SMART_INTERSECTION_OPERATOR_STAGE6_BEFORE_OUTPUT = $BeforePath
$env:SMART_INTERSECTION_OPERATOR_STAGE6_AFTER_OUTPUT = $AfterPath
$env:SMART_INTERSECTION_OPERATOR_STAGE6_CONTACT_SHEET = $ContactSheetPath
...
foreach ($path in @($BeforePath, $AfterPath)) {
  ...
  Set-Stage6PngOpaque -Path $path
}
New-Stage6ContactSheet -LeftPath $BeforePath -RightPath $AfterPath -OutputPath $ContactSheetPath
Set-Stage6PngOpaque -Path $ContactSheetPath
Write-Output "OPERATOR_STAGE6_BEFORE_OUTPUT=$BeforePath"
Write-Output "OPERATOR_STAGE6_AFTER_OUTPUT=$AfterPath"
