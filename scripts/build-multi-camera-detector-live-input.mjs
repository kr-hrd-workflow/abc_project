...
export async function buildMultiCameraDetectorLiveInputFile({
  detectorPaths,
  calibrationPath,
  signalPath,
  outputPath,
  ...
  if (
    !Array.isArray(detectorPaths) ||
    detectorPaths.length === 0 ||
    !calibrationPath ||
    !signalPath ||
    !outputPath
  ) {
    return {
      ...
      summary: null,
      output:
        "Usage: npm run real-sample:build-multi-camera-envelope -- <detector-output-a.json,detector-output-b.json> <camera-calibration.json> <signal-snapshot.json> <live-input-envelope.json>"
    };
  }

  try {
    const detectors = await Promise.all(
      detectorPaths.map(async (detectorPath) =>
        normalizeDetectorOutput(await readJsonFile(detectorPath, readFileImpl))
      )
    );
    ...
    const intersectionId = requireSharedIntersectionId(detectors);

    const cameraFrames = detectors.map((detector) => {
    ...
    const summary = {
      outputPath,
      intersectionId,
      cameraFrameCount: cameraFrames.length,
      detectionCount: detectors.reduce(
        (total, detector) => total + detector.detections.length,
        0
      ),
...
function requireSharedIntersectionId(detectors) {
  const [first] = detectors;
  const intersectionId = first?.intersectionId;
  if (
    !intersectionId ||
    detectors.some((detector) => detector.intersectionId !== intersectionId)
  ) {
    throw new Error("detector outputs must share one intersectionId");
  }
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  const [detectorPathsText, calibrationPath, signalPath, outputPath] =
    process.argv.slice(2);
  const result = await buildMultiCameraDetectorLiveInputFile({
    detectorPaths: detectorPathsText
      ? detectorPathsText.split(",").map((path) => path.trim()).filter(Boolean)
      : [],
    calibrationPath,
    signalPath,
    outputPath
