...
def test_korean_chat_wait_time_question_uses_simulation_metrics(
    client: TestClient,
) -> None:
    response = client.post(
        "/api/chat",
        json={
            "question": "시뮬레이션 결과 기준으로 대기시간이 얼마나 줄어?",
            "locale": "ko",
        },
    )
    ...
    assert response.status_code == 200
    payload = response.json()
    assert "평균 대기시간" in payload["answer"]
    assert "68.0초" in payload["answer"]
    assert "56.0초" in payload["answer"]
    assert "12.0초 감소" in payload["answer"]
