...
def test_stage4_renderer_snapshot_fixture_snapshots_expose_motion_binding_contract(
    client: TestClient,
) -> None:
    snapshot_a_response = client.get(
        "/api/renderer/unreal/snapshot"
        "?city_profile_id=seoul"
        "&stage4_fixture_id=stage4-fixture-a"
    )
    snapshot_b_response = client.get(
        "/api/renderer/unreal/snapshot"
        "?city_profile_id=seoul"
        "&stage4_fixture_id=stage4-fixture-b"
    )

    assert snapshot_a_response.status_code == 200
    assert snapshot_b_response.status_code == 200
    snapshot_a = snapshot_a_response.json()
    snapshot_b = snapshot_b_response.json()
    ...
    assert preserved_keys.issubset(snapshot_a)
    assert preserved_keys.issubset(snapshot_b)

    assert snapshot_a["snapshot_id"] == "stage4-fixture-a"
    assert snapshot_b["snapshot_id"] == "stage4-fixture-b"
    assert snapshot_a["motion_binding_version"] == "operator-stage4-motion-v1"
    assert snapshot_b["motion_binding_version"] == "operator-stage4-motion-v1"
    assert snapshot_a["simulation_source"] == "sumo_traci_fixture"
    assert snapshot_b["simulation_source"] == "sumo_traci_fixture"
    assert "No real traffic signal control" in snapshot_a["safety_boundary"]
    assert "No real traffic signal control" in snapshot_b["safety_boundary"]

    assert snapshot_a["activeSignalGroup"] == "east_west_priority"
    assert snapshot_b["activeSignalGroup"] == "north_south_priority"
    assert snapshot_a["cycleSecond"] != snapshot_b["cycleSecond"]
    assert snapshot_a["queues"] != snapshot_b["queues"]
    assert snapshot_a["pedestrianRequest"] is True
    assert snapshot_b["pedestrianRequest"] is False
    assert snapshot_a["emergencyVehicleDirection"] == "east"
    assert snapshot_b["emergencyVehicleDirection"] == "north"

    vehicles_a = snapshot_a["vehicles"]
    vehicles_b = snapshot_b["vehicles"]
    assert vehicles_a[0]["actor_label"] == (
    ...
    assert vehicles_a[0]["heading_deg"] != vehicles_b[0]["heading_deg"]
    assert {vehicle["actor_label"] for vehicle in vehicles_a} == {
        vehicle["actor_label"] for vehicle in vehicles_b
    }
    assert {
        signal["actor_label"] for signal in snapshot_a["signals"]
    } == {
        "OperatorStage3_Stage3SignalKit_SUMOReadyAssetPivot_"
        "seoul_northwest_pole",
        "OperatorStage3_Stage3SignalKit_SUMOReadyAssetPivot_"
        "seoul_southwest_pole",
    }
